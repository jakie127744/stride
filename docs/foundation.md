...
**Hosting decision (settled 16 Aug 2026):** Cloudflare R2 for storage + Cloudflare's own open-source [PMTiles Worker](https://github.com/protomaps/PMTiles/tree/main/serverless/cloudflare) in front of it. Reasoning: R2 has zero egress fees (the entire reason `tile.openstreetmap.org` bars embedded apps is bandwidth cost — S3-compatible storage with per-GB egress billing would just reintroduce that problem under a different vendor), a free tier (10GB storage, 10M reads/month) that comfortably covers city-sized extracts for a pre-revenue app, and the Worker handles PMTiles' HTTP range-request protocol correctly out of the box rather than needing bespoke range-request handling written for this project. Alternatives considered: raw S3/GCS (real egress cost at any scale — the thing PMTiles exists to avoid), Protomaps' own hosted service (good for prototyping, not a long-term production dependency for someone else's uptime/pricing). **This still needs a Cloudflare account, an R2 bucket, and the Worker actually deployed — that's account creation and (at scale) billing, which is the app owner's action to take, not something automatable from inside the codebase.**

**Client-side reading:** MapLibre Android 13.5.0 has no built-in PMTiles support (confirmed by inspecting the SDK jar — no `pmtiles`/`protomaps` classes). The standard approach elsewhere in the MapLibre ecosystem is a tiny loopback HTTP server on-device that parses the local `.pmtiles` file's directory structure and serves individual `{z}/{x}/{y}` tiles by byte range, with MapLibre's style pointed at `http://127.0.0.1:<port>/...` like any other tile server. `core/maps` (`PmtilesCache`/`PmtilesRepository`) currently implements the download-and-cache half of this (region detection, fetch, on-device storage) — the PMTiles binary-format reader and local tile server are real remaining engineering work, deliberately not guessed at without a real `.pmtiles` fixture to verify tile output against.
