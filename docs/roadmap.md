...
- [x] **Pulled forward — real vertical slice (ahead of Phase 4/5/6)**
  ...
  - Honest gaps this slice does NOT close: voice cues and location tracking both stop if the app is backgrounded (no foreground service yet — that's the real Phase 4 `MediaSessionService` architecture); no basemap tiles; Live Track is still a placeholder. Build-verified on device (Galaxy S23, Android 15), not just compiled.
